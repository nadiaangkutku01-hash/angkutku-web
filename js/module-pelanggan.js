// ============================================================================
// MODULE-PELANGGAN.JS -- Contoh migrasi modul lengkap
// ============================================================================
// Ini POLA yang akan diulang buat 19 modul lainnya. Perhatikan: TIDAK
// ADA lagi pengecekan "kalau Role Marketing, filter cuma miliknya
// sendiri" di sini -- itu SEKARANG otomatis ditegakkan oleh RLS
// (Fase 2, kebijakan "pelanggan_select"). Query di bawah SELALU minta
// "semua data", tapi Supabase cuma balikin yang memang boleh dilihat
// sesuai Role yang login -- jauh lebih SEDERHANA & AMAN dibanding kode
// GAS lama yang harus filter manual di JavaScript.
// ============================================================================
...
window.renderPelangganModule = async function(area) {
  ...
  const canInput = window.CURRENT_USER_SESSION.role !== 'Investor'; // contoh aturan sederhana, sesuaikan kalau perlu

  // Query LANGSUNG dari client ke Supabase -- gak ada lagi lapisan
  // backend/globalCreate/globalUpdate di tengah seperti GAS. RLS yang
  // jaga keamanannya.
  const { data: rows, error } = await supabaseClient.from('pelanggan').select('*').order('created_at', { ascending: false });
  ...
  area.innerHTML = `
    <div class="bg-white rounded-2xl border border-slate-200 shadow-xs overflow-hidden">
      ...
      ${rows.length === 0 ? '<div class="p-8 text-center text-slate-400 italic text-xs">Belum ada data (atau memang gak ada yang bisa kamu lihat sesuai hak aksesmu).</div>' : ''}
    </div>`;
};
...
window.pelangganSubmit = async function(idPelanggan) {
  const payload = {
    ...
    kota_domisili: document.getElementById('f-kota').value,
  };

  let error;
  if (idPelanggan) {
    // EDIT -- RLS pastikan cuma bisa edit kalau memang boleh (Owner/
    // Admin/dst, atau Marketing buat miliknya sendiri).
    ({ error } = await supabaseClient.from('pelanggan').update(payload).eq('id_pelanggan', idPelanggan));
  } else {
    // TAMBAH BARU -- generate ID sederhana, dan catat dibuat_oleh
    // OTOMATIS dari sesi yang login (pengganti window.CURRENT_USER_
    // SESSION.name yang dulu ditulis manual tiap create di GAS).
    payload.id_pelanggan = 'PLG-' + Date.now();
    payload.dibuat_oleh = window.CURRENT_USER_SESSION.name;
    ({ error } = await supabaseClient.from('pelanggan').insert(payload));
  }

  if (error) { alert('Gagal simpan: ' + error.message); return; }

  document.getElementById('pelanggan-modal').remove();
  renderPelangganModule(document.getElementById('content-area'));
};

window.pelangganDelete = async function(idPelanggan) {
  if (!confirm('Hapus data ini?')) return;
  const { error } = await supabaseClient.from('pelanggan').delete().eq('id_pelanggan', idPelanggan);
